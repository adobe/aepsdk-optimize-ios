...
public class ExperienceData: NSObject {
    ...
    internal func asDictionary() -> [String: Any]? {
        ...
        dictionary[OptimizeConstants.EventDataKeys.XDM] = xdm

        if let data = data {
            dictionary[OptimizeConstants.EventDataKeys.DATA] = data
        }
        if let datasetIdentifier = datasetIdentifier {
            dictionary[OptimizeConstants.EventDataKeys.DATASET_ID] = datasetIdentifier
        }
